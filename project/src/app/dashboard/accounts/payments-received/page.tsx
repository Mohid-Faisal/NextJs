...
type SortField = "date" | "amount" | "fromAccount" | "mode";
...
export default function PaymentsReceivedPage() {
  ...
  const filterAndSortPayments = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(p => 
        p.fromAccount?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "amount":
          ...
          break;
        case "fromAccount":
          aValue = a.fromAccount?.toLowerCase() || "";
          bValue = b.fromAccount?.toLowerCase() || "";
          break;
        case "mode":
  ...
  const handleExportExcel = () => {
    const headers = ["Date", "From", "Amount", "Mode", "Invoice", "Reference", "Description"];
    ...
    exportToExcel(data, headers, 'payments_received');
  };
  ...
  const handleExportPrint = async () => {
    ...
    await exportToPrint(data, headers, 'Payments Received Report');
  };
  ...
  const handleExportPDF = () => {
    ...
    exportToPDF(data, headers, 'Payments Received Report');
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            View all income payments received from customers
          </p>
          <p className="text-sm text-blue-600 dark:text-blue-400 mt-1 font-medium">
            Showing all payments received
          </p>
        </div>

        {/* Read-only stats tabs on the top right */}
        <div className="flex items-center gap-3 bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
          <div className="px-4 py-2 text-xs sm:text-sm font-medium rounded-md flex flex-col items-center justify-center min-w-[140px] bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 shadow-sm border border-gray-150/40">
                ...
                <tbody className="text-sm text-gray-700 dark:text-gray-200">
                  {filteredPayments.map((payment) => (
                    <tr key={payment.id} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800">
                      ...
                      <td className="px-4 py-3 font-medium">{payment.fromAccount || "-"}</td>
