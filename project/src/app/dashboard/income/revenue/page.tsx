...
export default function IncomeRevenuePage() {
          ...
          try {
            ...
            if (response.ok) {
              const foundInvoice = (data.invoices || []).find(
                (inv: Invoice) => inv.invoiceNumber === invoiceParam && inv.profile === "Customer"
              );
              if (foundInvoice) {
  ...
  const updateAccountsBasedOnPaymentMethod = (accounts: ChartOfAccount[], paymentMethod?: string) => {
    ...
    // Credit account is always Accounts Receivable for customer payments
    ...
    switch (method) {
      ...
      case "BANK_TRANSFER":
        ...
        break;
      case "CHECK":
        // Checks are typically deposited to bank, but can be cash
        if (bankAccount) {
          setDebitAccountId(bankAccount.id);
        } else if (cashAccount) {
          setDebitAccountId(cashAccount.id);
        ...
        break;
      case "CREDIT_CARD":
        // Credit card payments typically go to bank account
        if (bankAccount) {
          setDebitAccountId(bankAccount.id);
        } else if (cashAccount) {
          setDebitAccountId(cashAccount.id);
    ...
    <div className="w-full min-h-full p-4 sm:p-6 lg:p-8 xl:p-10 bg-white dark:bg-zinc-900">
      ...
      <div className="w-full">
        ...
        <Dialog open={paymentDialogOpen} onOpenChange={setPaymentDialogOpen}>
          <DialogContent size="2xl" className="max-w-6xl w-full max-h-[90vh] overflow-y-auto">
            <DialogHeader>
