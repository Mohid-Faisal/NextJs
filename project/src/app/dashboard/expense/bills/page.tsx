...
export default function ExpenseBillsPage() {
  ...
  useEffect(() => {
    ...
    const fetchInvoices = async () => {
      try {
        ...
        console.log('Filtered Vendor Invoices:', vendorInvoices.length, 'out of', json.invoices?.length || 0);
    ...
    fetchInvoices();
  }, [page, pageSize, statusFilter, searchTerm, dateRange, sortField, sortOrder, selectedBranch, refreshKey]);
  ...
  const handleBulkStatusChange = async (newStatus: string) => {
    ...
    const toastId = toast.loading(`Updating status of ${selectedInvoiceIds.length} bill(s)...`);
    ...
    if (successCount > 0) {
      toast.success(`Successfully updated ${successCount} bill(s) to ${newStatus}.${failCount > 0 ? ` (${failCount} failed)` : ""}`);
      setSelectedInvoiceIds([]);
    ...
    <div className="p-4 sm:p-6 lg:p-8 xl:p-10 w-full bg-white dark:bg-zinc-900 transition-all duration-300 ease-in-out ml-0 lg:ml-0">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-4">
        ...
        <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 rounded-lg p-1 flex-wrap">
          ...
          <div
            aria-disabled
            className="px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium rounded-md flex flex-col items-center justify-center bg-orange-50 dark:bg-orange-900/30 border border-orange-200 dark:border-orange-800 min-w-[120px] cursor-default select-none"
          >
            <span className="text-lg sm:text-xl font-bold text-orange-700 dark:text-orange-300">
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-2 text-sm font-bold text-orange-400 hover:text-orange-300 transition-colors cursor-pointer bg-transparent border-0 disabled:opacity-50"
              >
