...
async function main() {
  console.log(`=== SEQUENTIAL INVOICE RECONCILIATION (${APPLY ? "APPLY" : "DRY RUN"}) ===\n`);
  ...
  for (const org of orgs) {
    ...
    if (APPLY && missingInvoices.length > 0) {
      console.log(`Creating ${missingInvoices.length} missing journal entries...`);
      const BATCH_SIZE = 25;
      
      for (let i = 0; i < missingInvoices.length; i += BATCH_SIZE) {
        const chunk = missingInvoices.slice(i, i + BATCH_SIZE);
        ...
        if ((i + BATCH_SIZE) % 500 === 0 || i + BATCH_SIZE >= missingInvoices.length) {
          console.log(`Created ${Math.min(i + BATCH_SIZE, missingInvoices.length)} / ${missingInvoices.length} JEs`);
        }
      }
